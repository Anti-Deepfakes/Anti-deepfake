...
async def generate_disrupt_v1(model, image, device, face_detector):
    ...
    image_tensor, image_weighted = await preprocessing(image_path, face_detector)
    ...
    try:
        ...
        with torch.no_grad():
            ...
            perturbations = F.interpolate(perturbations, size=(image_tensor.shape[2], image_tensor.shape[3]),
                                          mode='area')
            perturbed_images = image_tensor + perturbations
