...
async def preprocessing(image_path, face_detector):
    ...
    return image_tensor.unsqueeze(0), image_weighted.unsqueeze(0)
...
async def add_weight_to_landmarks(weight_map, landmarks):
    if len(landmarks) > 0:
        for (x, y, _) in landmarks:
            x, y = int(x), int(y)
            weight_map[max(0, y - 5):min(weight_map.shape[0], y + 5),
            max(0, x - 5):min(weight_map.shape[1], x + 5)] = 2.0  # 랜드마크 주변에 가중치 2 부여
    return weight_map
